...
- 戻る操作は左上の大きな「←」1 箇所のみ。
- **答え方を切り替えさせない。** タップの手段は問題ごとに 1 つ。声は切り替えではなく、常に横で聞いている。
- **数字の自由入力を置かない。** 「40じ」が入力できてしまう。時刻も数も選択肢と声で答える。
- **音は順番に。** 効果音 → 読み上げ → 聞き取り。重ねない。
- **やめる確認は絵と声で。** システムのダイアログ（文字だけ）は使わない。
...
HomeView
  ...
  → 問題 1 …… n （各問 20〜40 秒）
       ├ 出題（イラスト＋音声）→ 読み終えたら聞き取り開始
       ├ 回答（選択 / ドラッグ / なぞり）＋ 声（同時に受け付ける）
       ├ 正解 → ✨演出 + 「せいかい！」+ ★獲得 → 読み終えたら自動で次へ（「つぎへ」で先にも進める）
...
マイクのボタンは押させない。3〜6歳に「押してから話す」「答え方を切り替える」は無理。

```
問題表示
  → 問いかけを読み上げ（この間はマイクを開かない：自分の声を拾ってしまう）
  → 読み終えたら自動で聞き取り開始（キャラが耳をかたむける・波形・「はなしてね」）
       ├ 正解        → 正解演出
       ├ 不正解      → 「おしい！もういっかい！」（学習履歴には不正解として記録）
       ├ 認識できない → 「もういちど いってみよう！」→ 読み終えたらまた聞く（不正解として記録しない）
       │                 3回続いたら「タップで こたえても いいよ！」
       └ 何も聞こえない → 何も言わずにもう一度聞く（黙っている子を急かさない）。
                          3 回続いたら聞き取りを止め、帯がマイクのボタンになる（押せばまた聞く）
  → 権限が未取得ならオンボーディングで先に頼んである。拒否されていればはじめから聞かない
```

問題は 2 種類ある。`answerModes` の先頭で決まる。

| 型 | 例 | 画面 |
| --- | --- | --- |
| タップ優先（先頭が `.choice` など） | いくつかな？ / なんじかな？ / 「あ」はどれ？ | 選択肢を出したまま、上の帯で聞いている。押しても言ってもよい |
| 声優先（先頭が `.voice`） | この もじは なんて よむ？ / この すうじ なんて よむ？ / えいごで いってみよう | 答えになる選択肢は隠す。黙ったまま・聞き取れないまま続いたら、声で知らせてタップに切り替える（`Prompt.tapFallbackSpokenText`）。「タップで こたえる」ボタンでも切り替えられる |

読み上げには完了コールバックがある（`SpeechSynthesizing.speak(_:locale:volume:completion:)`）。
「読み終えてから聞く」「効果音が鳴り終わってから話す」「読み終えてから次へ」はすべてこれで順序づける。
...
- 今日のチャレンジ（複数教科ミックス）
- とけい：時刻を読む（選択 / 声）
- とけい：針をドラッグして合わせる（長い針を回すと短い針もつながって動く）
- ひらがな：読み・なぞり書き・ことばあわせ・音声回答
- カタカナ：読み・なぞり書き・音声回答
- すうじ：かぞえる・数字を読む（声優先）・位（一/十/百）
- アルファベット：大文字小文字・なぞり書き・音声回答
- えいごのことば：絵と音と文字・音声回答
- ご飯タイマー（キャラクターとの競争）
- 共通の音声入力 UI（自動で聞き取り開始・波形・キャラリアクション・タップへの自動切り替え）
- 学習結果の保存（SwiftData）
