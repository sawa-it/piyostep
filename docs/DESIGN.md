...
起動
 └─ RootView（オンボーディング or ホーム）
     ...
     │     3. あいぼうをえらぶ（キャラクター選択）
     └─ HomeView（子どもモード / タブなし・大ボタン）
          ├─ きょうのチャレンジ   → SessionView →（結果画面なしで）HomeView
          ...
          ├─ きょうは おしまい    → DayEndView（その日の★と新しく手に入ったものをまとめて受け取る）
...
HomeView
  ...
  → ひとこと ねぎらって HomeView（結果画面は挟まない）
```

★や新しく手に入ったものは **1 回ごとには受け取らない**。
問題を解き終えるたびに「受け取る画面」が挟まると、幼児には待ち時間でしかない。
その日の遊び終わりに、ホームの「きょうは おしまい」（DayEndView）でまとめて受け取る。

```
HomeView → [きょうは おしまい]
  → DayEndView
       ├ きょうの★（前回の「おしまい」以降のぶん）・やった問題数
       ├ あたらしく てにいれたもの（前回の「おしまい」以降に解放されたもの）
       └ 「おやすみ！ また あした」→ おしまいの時刻を保存 → HomeView
```

「おしまい」を押さずに日をまたいでも、解放されたものは次の「おしまい」で見せる
（DayEndSummarizer は ★を「きょう」、アンロックを「前回のおしまい以降」で数える）。

### 3.2 音声回答フロー（全教科共通）

**押すものは無い。** 「マイクを押してから話す」も「音声モードに切り替える」も幼児にはできない。
音声は回答方法のひとつではなく、タップで答える UI の横で いつも聞いている。

```
問題表示（タップで答える UI が 1 つだけ出る。切り替えは無い）
  → 問題を読み上げる（この間マイクは閉じておく。自分の声を答えとして拾わないため）
  → 読み終えたら自動でマイクを開く（音声が使える問題・設定 ON・許可済みのとき）
  → 「きいているよ」のしるし（キャラが耳をかたむける + 波形。ボタンではない）
  → 発話 → 認識結果を正規化 → 判定
       ├ 正解        → 正解演出
       ├ 不正解      → 「おしい！もういっかい！」（学習履歴には不正解として記録）
       └ 認識できない → 静かに聞き直す（不正解として記録しない・言い返さない）
  → 黙っている間も 20 秒ごとに開き直して待ち続ける
  → タップで答えたら、その場でマイクを閉じる（遅れて届いた結果は無視）
```

- マイクの許可はオンボーディングで頼む。答える場面でダイアログを割り込ませない
  （未決定のままなら、その問題では音声を使わず タップで進める）。
- まわりの話し声も拾うので、聞き取れなかったときに いちいち言い返さない。
- マイクが開けない失敗が 3 回続いたら、その問題では諦める（タップで答えられる）。
...
│  ・unclear / 声なし のあとは ViewModel が自動で聞き直す   │
│  ・権限拒否 / 未対応端末は即 .unavailable（学習は継続可） │
│  ・requestingPermission は使わない（許可は起動時に取る）  │
└─────────────────────────────────────────────────────────┘
        ▲ 読み上げ完了 → 自動で listening（SessionViewModel）
        ▲ 依存は protocol のみ（テストではモック）
        │
┌───────┴────────────┐        ┌─────────────────────────┐
│ SpeechRecognizing  │        │ SpeechSynthesizing      │
│  .authorize()      │        │  .speak(text, locale,   │
│  .start(locale:)   │        │         completion:)    │
│  .stop()           │        │  .stop()                │
│                    │        └─────────────────────────┘
│  結果: AsyncStream │                 ▲
└────────────────────┘                 │
...
- とけい：針をドラッグして合わせる
- ひらがな：読み・なぞり書き・かきとり（Lv4〜。お手本なし。まちがえた 2 回目だけ うすい お手本をヒントに出す）・ことばあわせ・音声回答
- カタカナ：読み・なぞり書き・音声回答
- すうじ：かぞえる・数字を読む・位（一/十/百）・音声回答
- アルファベット：大文字小文字・なぞり書き・音声回答
- えいごのことば：絵と音と文字・音声回答
- ご飯タイマー（キャラクターとの競争）
- 共通の音声入力（読み上げ後に自動で聞く・波形・キャラリアクション。押すものは無い）
- きょうは おしまい（その日の★と新しく手に入ったものを、まとめて受け取る）
- 学習結果の保存（SwiftData）
