...
- アルファベット・超基本英単語
- **キャラクターと競争するご飯タイマー**
- 音声入力での回答（日本語／英語。ボタンは押さず、読み上げのあと自動で聞く）
- 「きょうは おしまい」で、その日の★と新しく手に入ったものをまとめて受け取る
- ずかん・スタンプ・きせかえのアンロック
...
| ViewModel・サービス | `PiyoStepTests` | セッション進行、音声回答（読み上げ後に自動で聞く・モック認識）、きょうは おしまい、ご飯タイマー、保護者ゲート、設定、SwiftData 永続化、広告の抑止（保護者画面に入るときだけ出す）|
| 主要導線 | `PiyoStepUITests` | 起動・プロフィール作成・ホーム・今日のチャレンジ・時計/数字/ひらがな・なぞり書き・音声回答（押さずに聞く）・きょうは おしまい・ご飯タイマー・保護者画面・設定変更 |
